...
if str2double(date) == 20150608
    NISfile = 'NIS01_20150608_165842_rdn';
    tarp3_coord = [304,307;364,369];
    tarp48_coord = [313,319;365,370];
    veg_coord = [324,328;380,420];
    EWroad_coord = [310,348;354,356];
    NSroad_coord = [334,336;317,354];
    filename = {'NEON_20150608_Baseline',...
        'NEON_20150608_1kmAtmo_40H2O'};
    SunEllipticFactor = 0.97083;    % Determined by Day of Year
    mu = cosd(31);                  % Determined by Time of Day
...
%     SSIRfile = '/Users/wrightad/Documents/Data/NEON/FlightData/20150608_SSIR_CALIB_0602.mat';
    SSIRfile = '20150608_SSIR_CALIBSPECT_Jul2017.mat';
    asdfile = '20150608_ASD_REFL.mat';

elseif str2double(date) == 20150616
    NISfile = 'NIS01_20150616_151613_rdn';
    tarp3_coord = [140,145;232,237];
    tarp48_coord = [150,155;232,237];
    veg_coord = [155,160;253,280];
    EWroad_coord = [148,185;220,224];
    NSroad_coord = [168,172;182,218];
    filename = {'NEON_20150616_Baseline',...
        'NEON_20150616_259mAtmo_30H2O'};
    SunEllipticFactor = 0.96950;    % Determined by Day of Year
    mu = cosd(50.2);                % Determined by Time of Day
    SSIRfile = '20150616_SSIR_CALIBSPECT_20150616FIELDB.mat';
    asdfile = '20150616_ASD_REFL.mat';
%     asdfile = '/Users/wrightad/Documents/Data/NEON/GroundData/ASD_Refl/20150608_ASD_REFL.mat';

elseif str2double(date) == 20150617
    NISfile = 'NIS01_20150617_203142_rdn';
    tarp3_coord = [174,180;200,205];
    tarp48_coord = [183,190;200,207];
    veg_coord = [196,199;220,245];
    EWroad_coord = [173,226;190,192];
    NSroad_coord = [208,211;159,190];
    filename = {'NEON_20150617_Baseline',...
        'NEON_20150617_1kmAtmo_40H2O'};
    SunEllipticFactor = 0.96933;    % Determined by Day of Year
    mu = cosd(25.0);                % Determined by Time of Day
    SSIRfile = '20150617_SSIR.mat';
    asdfile = '20150617_ASD_REFL.mat';
...
respfunc = importdata('nis2013_fmed2_2013.dat');
...
data = importdata('SUN01med2irradwnNormt.dat',' ',2);
...
[t_freq, t_ts, t_Tso, t_t, t_T, t_sph_alb] = load_acd(strcat(filename{1},'.acd'));
t_Ts = t_Tso./t_T;
t_Ts(isnan(t_Ts)) = 0;
t_Ts(isinf(t_Ts)) = 0;
wvl = 1e7./(t_freq);
...
% Load MODTRAN Irradiance
flx_data = importdata(strcat(filename{1},'.flx'),' ', 16);
flx_data.data = flx_data.data(1:4351,:);
...
data_7sc = importdata(strcat([filename{1},'.7sc']),' ',11);
wvl_7sc = data_7sc.data(1:end-1,1);
r0 = data_7sc.data(:,9);
...
t_Ts_1km(isnan(t_Ts_1km)) = 0;
t_Ts_1km(isinf(t_Ts_1km)) = 0;
...
flx_data_1km = importdata(strcat(filename{2},'.flx'),' ',16);
flx_data_1km.data = flx_data_1km.data(1:4351,:);
t_T_derived_1km = flx_data_1km.data(:,4)./flx_data_1km.data(:,34);
t_Ts_derived_1km = flx_data_1km.data(:,4)./flx_data_1km.data(:,end);
t_T_derived = flx_data.data(:,4)./flx_data.data(:,34);
t_Ts_derived = flx_data.data(:,4)./flx_data.data(:,end);

T_derived_1km = super_resample(flx_data_1km.data(:,4)./flx_data_1km.data(:,34),flx_data_1km.data(:,1),neon_wvl,neon_fwhm);
Ts_derived_1km = super_resample(flx_data_1km.data(:,4)./flx_data_1km.data(:,end),flx_data_1km.data(:,1),neon_wvl,neon_fwhm);
...
ssim_zfwhm0 = [365.25, 7.67;
               ...
               1791.46, 10.79;
               2061.62, 17.58];
ssim_zfwhm = zeros(439,1);
ssim_zfwhm(1:192) = interp1(ssim_zfwhm0(1:5,1),ssim_zfwhm0(1:5,2),ssim_zwvl(1:192),'linear','extrap');
ssim_zfwhm(193:end) = interp1(ssim_zfwhm0(6:end,1),ssim_zfwhm0(6:end,2),ssim_zwvl(193:end),'linear','extrap');
ssim_nwvl = nwvl;
...
plot(neon_wvl,Ig_NIS,'Color',[0,0,0])
plot(zwvl,Ig_ssim,'Color',[33,102,172]./255)
plot(neon_wvl,Ig_ssim_resampled,'Color',[178,24,43]./255)



legend('NIS Sampling','SSIM Sampling','SSIM Resampled to NIS')
ylabel('Spectral Irradiance [W m^{-2} nm^{-1}]');
xlabel('Wavelength [nm]')
title('Modelled Downwelling Irradiance')
set(gca,'FontSize',18)
axis([720,800,0.7,1.3]);grid on;box on
print('IrradianceDiffRespFunc_O2.svg','-dsvg')
axis([350,1025,0,1.8]);
print('IrradianceDiffRespFunc_Full.svg','-dsvg')

figure;
plot(neon_wvl,Ig_NIS-Ig_ssim_resampled,'k')
ylabel('Spectral Irradiance [W m^{-2} nm^{-1}]');
xlabel('Wavelength [nm]')
title('Difference Between NIS and SSIM Sampling')
axis([720,800,-0.15,0.1]);grid on;box on
set(gca,'FontSize',18)
print('DiffRespFunc_O2.svg','-dsvg')

% Load Ground Reflectances
load('20150617_ASD_REFL.mat')
...
figure;subplot(2,2,1);plot(neon_wvl,tarp3_asd17,'k--');hold on;
plot(neon_wvl,tarp3_asd16,'k:');
plot(neon_wvl,tarp3_asd08,'k');
subplot(2,2,2);plot(neon_wvl,tarp48_asd17,'b--');hold on
plot(neon_wvl,tarp48_asd16,'b:');
plot(neon_wvl,tarp48_asd08,'b');
subplot(2,2,3);plot(neon_wvl,veg_asd17,'Color',[34,139,34]./255,'LineStyle','--');hold on
plot(neon_wvl,veg_asd16,'Color',[34,139,34]./255,'LineStyle',':');
plot(neon_wvl,veg_asd08,'Color',[34,139,34]./255);
subplot(2,2,4);plot(neon_wvl,ewroad_asd17,'Color',[156,102,31]./255,'LineStyle','--');hold on
plot(neon_wvl,ewroad_asd16,'Color',[156,102,31]./255,'LineStyle',':');
plot(neon_wvl,ewroad_asd08,'Color',[156,102,31]./255);
legend('Jun 17','Jun 16','Jun 8')
...
[~,ssirtime_ind] = min(abs(ssirtime-obs_time(1)));
...
figure;hold on;
plot(neon_wvl0,super_resample(flx_fdn/mu,flx_wvl,neon_wvl0,neon_fwhm0)*10000,'r');
plot(zwvl,Ifdn_obs_trim,'k');

model_Ifdn = super_resample(flx_fdn/mu,flx_wvl,neon_wvl0,neon_fwhm0)*10000;
Ifdn = mu.*I0.*super_resample((t_Ts + t_ts)./(t_T + t_t),wvl,neon_wvl,neon_fwhm);
model_Ifup = super_resample(flx_fup/mu,flx_wvl,neon_wvl0,neon_fwhm0)*10000;
obs_Ifdn = Ifdn_obs_trim;

save(strcat(num2str(date),'_downirradcomp.mat'),'model_Ifdn','obs_Ifdn','neon_wvl0','zwvl')

%% Section 2: Processing
% Extract Ground Spectra from NIS Scene
tarp3_spc = reshape(data(tarp3_coord(2,1):tarp3_coord(2,2),tarp3_coord(1,1):tarp3_coord(1,2),:),[],426);
tarp48_spc = reshape(data(tarp48_coord(2,1):tarp48_coord(2,2),tarp48_coord(1,1):tarp48_coord(1,2),:),[],426);
veg_spc = reshape(data(veg_coord(2,1):veg_coord(2,2),veg_coord(1,1):veg_coord(1,2),:),[],426);
EWroad_spc = reshape(data(EWroad_coord(2,1):EWroad_coord(2,2),EWroad_coord(1,1):EWroad_coord(1,2),:),[],426);
NSroad_spc = reshape(data(NSroad_coord(2,1):NSroad_coord(2,2),NSroad_coord(1,1):NSroad_coord(1,2),:),[],426);
...
Tt = super_resample((t_T + t_t),wvl,neon_wvl,neon_fwhm);
...
Tsts_T = super_resample((t_Ts + t_ts).*t_T,wvl,neon_wvl,neon_fwhm);
Tsts_t = super_resample((t_Ts + t_ts).*t_t,wvl,neon_wvl,neon_fwhm);
...
% Standard Retrieval with ADJACENCY Correction
        stand_tarp3_ret_noadj_mu(j,:) = pi.*(Rob-rad0)./(sph_alb.*pi.*(Rob-rad0)+mu.*I0.*(Tsts_Tt));

    
    rho_bar_ret_mu = (Ifup_obs_trim - Ifup0) ./ ((Ifdn .* ...
        (t_1km.^2 + (1 + mu) * t_1km .* sqrt(Tso_1km) + mu.*Tso_1km) + (Ifup_obs_trim - Ifup0) .* sph_alb_1km));
      stand_tarp3_ret_mu(j,:) = (Rob - rad0 - (model_Ifdn(1:152) .* (t_1km + sqrt(Tso_1km)) .* t_1km) ./ ...
          pi .* (rho_bar_ret_mu ./ (1 - rho_bar_ret_mu .* sph_alb_1km))) .* ...
          (pi ./ (model_Ifdn(1:152) .* (t_1km + sqrt(Tso_1km)) .* mu .* sqrt(Tso_1km))) .* (1 - rho_bar_ret_mu .* sph_alb_1km);
...
plot(asd_wvl,mean(tarp03,1)'./100,'k','LineWidth',1.5)
...
plot(neon_wvl,mean(tarp3_ret_TOA_B,1),'-','Color',standard,'LineWidth',1.5)
plot(neon_wvl,mean(tarp3_ret_noadj,1),'Color',intermediate,'LineWidth',1.5) % Intermediate
plot(neon_wvl,mean(tarp3_ret,1),'Color',enhanced,'LineWidth',1.5)
plot(neon_wvl,mean(stand_tarp3_ret_noadj_mu,1),'Color',stand_adj,'LineWidth',1.5) % Intermediate
plot(neon_wvl,mean(stand_tarp3_ret_mu,1),'Color',stand_adj,'LineStyle','--','LineWidth',1.5)
ylabel('Reflectance','FontSize',12);xlabel('Wavelength [nm]','FontSize',12);
axis([350,1025,0,0.1])
set(gca,'FontSize',12,'XColor','k','YColor','k');box on;
fig = gcf;
set(fig,'PaperUnits','inches','PaperPosition',[0 0 4 4]);
print(fig,'-depsc','-r300','TestTarp3Retrieval.eps')
...
plot(neon_wvl,(mean(tarp3_ret_TOA_B,1)-tarp03_r)*100,'Color',standard);grid on;
...
for j = 1:n
    ...
    Ifup0 = super_resample(rho_a_I,flx_wvl,neon_wvl,neon_fwhm).*Ifdn_obs_trim;
          ...
          % Standard Retrieval with ADJACENCY Correction
%       stand_tarp48_ret_noadj_mu(j,:) = pi .* (Rob - rad0) ./ (sph_alb_1km .* pi .* ...
%           (Rob - rad0) + model_Ifdn(1:152) .* (t_1km.^2 + (1 + mu) * t_1km .* sqrt(Tso_1km) + mu.*Tso_1km));
        stand_tarp48_ret_noadj_mu(j,:) = pi.*(Rob-rad0)./(sph_alb.*pi.*(Rob-rad0)+mu.*I0.*(Tsts_Tt));

    
%     rho_bar_ret_mu = (Ifup_obs_trim - Ifup0) ./ ((Ifdn .* mu .* Tt.^2 + ...
%         (Ifup_obs_trim - Ifup0) .* sph_alb_1km));
    rho_bar_ret_mu = veg_asd08;
      stand_tarp48_ret_mu(j,:) = (pi.*(sph_alb .* rho_bar_ret_mu + 1) .* (Rob - rad0) - (mu.*I0 .* Tsts_t) .* (sph_alb .* rho_bar_ret_mu)) ./ ...
            (mu.*I0 .* Tsts_T);
      
%     rho_bar_ret_mu = (Ifup_obs_trim - Ifup0) ./ ((Ifdn .* ...
%         (t_1km.^2 + (1 + mu) * t_1km .* sqrt(Tso_1km) + mu.*Tso_1km) + (Ifup_obs_trim - Ifup0) .* sph_alb_1km));
%       stand_tarp48_ret_mu(j,:) = (Rob - rad0 - (model_Ifdn(1:152) .* (t_1km + sqrt(Tso_1km)) .* t_1km) ./ ...
%           pi .* (rho_bar_ret_mu ./ (1 - rho_bar_ret_mu .* sph_alb_1km))) .* ...
%           (pi ./ (model_Ifdn(1:152) .* (t_1km + sqrt(Tso_1km)) .* mu .* sqrt(Tso_1km))) .* (1 - rho_bar_ret_mu .* sph_alb_1km);
...
plot(neon_wvl,mean(stand_tarp48_ret_noadj_mu,1),'Color',stand_adj,'LineWidth',1.5) % Intermediate
plot(neon_wvl,mean(stand_tarp48_ret_mu,1),'Color',stand_adj,'LineStyle','--','LineWidth',1.5)
ylabel('Reflectance','FontSize',12);xlabel('Wavelength [nm]','FontSize',12);
axis([350,1025,0,1])
...
for j = 1:n
    ...
    veg_ret(j,:) = (Rob - rad0 - (Ifdn_obs_trim .* (t_1km + sqrt(Tso_1km)) .* t_1km) ./ ...
      ...
      % TEST UPWELLING ADJACENCY
        stand_veg_ret_noadj_mu(j,:) = pi.*(Rob-rad0)./(sph_alb.*pi.*(Rob-rad0)+mu.*I0.*(Tsts_Tt));

    
    rho_bar_ret_mu = (Ifup_obs_trim - Ifup0) ./ ((Ifdn .* ...
        (t_1km.^2 + (1 + mu) * t_1km .* sqrt(Tso_1km) + mu.*Tso_1km) + (Ifup_obs_trim - Ifup0) .* sph_alb_1km));
      stand_veg_ret_mu(j,:) = (Rob - rad0 - (model_Ifdn(1:152) .* (t_1km + sqrt(Tso_1km)) .* t_1km) ./ ...
          pi .* (rho_bar_ret_mu ./ (1 - rho_bar_ret_mu .* sph_alb_1km))) .* ...
          (pi ./ (model_Ifdn(1:152) .* (t_1km + sqrt(Tso_1km)) .* mu .* sqrt(Tso_1km))) .* (1 - rho_bar_ret_mu .* sph_alb_1km);
...
plot(neon_wvl,mean(stand_veg_ret_noadj_mu,1),'Color',stand_adj,'LineWidth',1.5) % Intermediate
plot(neon_wvl,mean(stand_veg_ret_mu,1),'Color',stand_adj,'LineStyle','--','LineWidth',1.5)
ylabel('Reflectance','FontSize',12);xlabel('Wavelength [nm]','FontSize',12);
axis([350,1025,0,0.4])
...
for j = 1:n
    ...
    EWroad_ret(j,:) = (Rob - rad0 - (Ifdn_obs_trim .* (t_1km + sqrt(Tso_1km)) .* t_1km) ./ ...
      ...
      % Standard Retrieval with ADJACENCY Correction
        stand_EWroad_ret_noadj_mu(j,:) = pi.*(Rob-rad0)./(sph_alb.*pi.*(Rob-rad0)+mu.*I0.*(Tsts_Tt));

    
    rho_bar_ret_mu = (Ifup_obs_trim - Ifup0) ./ ((Ifdn .* ...
        (t_1km.^2 + (1 + mu) * t_1km .* sqrt(Tso_1km) + mu.*Tso_1km) + (Ifup_obs_trim - Ifup0) .* sph_alb_1km));
      stand_EWroad_ret_mu(j,:) = (Rob - rad0 - (model_Ifdn(1:152) .* (t_1km + sqrt(Tso_1km)) .* t_1km) ./ ...
          pi .* (rho_bar_ret_mu ./ (1 - rho_bar_ret_mu .* sph_alb_1km))) .* ...
          (pi ./ (model_Ifdn(1:152) .* (t_1km + sqrt(Tso_1km)) .* mu .* sqrt(Tso_1km))) .* (1 - rho_bar_ret_mu .* sph_alb_1km);
...
plot(neon_wvl,mean(stand_EWroad_ret_noadj_mu,1),'Color',stand_adj,'LineWidth',1.5) % Intermediate
plot(neon_wvl,mean(stand_EWroad_ret_mu,1),'Color',stand_adj,'LineStyle','--','LineWidth',1.5)
ylabel('Reflectance','FontSize',12);xlabel('Wavelength [nm]','FontSize',12);
