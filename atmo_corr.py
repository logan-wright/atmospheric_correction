...
def atmcorr(wvl,radiance,model,irrad_up = None,irrad_down = None):
    ...
    for j = 1:n:
        ...
        # Standard TOA Retrieval
        refl_TOA[j,:] = pi.*(Rob-rad0)./(sph_alb.*pi.*(Rob-rad0)+mu.*I0.*((Ts + ts).*(T + t)));
        refl_TOA_B[j,:] = pi.*(Rob-rad0)./(sph_alb.*pi.*(Rob-rad0)+mu.*I0.*Tsts_Tt);
